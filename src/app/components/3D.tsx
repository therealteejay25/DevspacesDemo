...
const secondRow = reviews.slice(3, 7);
const thirdRow = reviews.slice(0, 4);
const fourthRow = reviews.slice(3, 7);

export function Marquee3D() {
  return (
    <div className="relative flex h-96 w-full flex-row items-center justify-center gap-4 overflow-hidden [perspective:300px]">
      <div className="flex flex-row items-center gap-4" style={{ transform: "translateX(-50px) translateZ(-100px) rotateX(20deg) rotateY(-10deg) rotateZ(20deg)" }}>
        <Marquee pauseOnHover vertical className="[--duration:25s]">{firstRow.map((r) => <ReviewCard key={r.username} {...r} />)}</Marquee>
        <Marquee reverse pauseOnHover vertical className="[--duration:25s]">{secondRow.map((r) => <ReviewCard key={r.username} {...r} />)}</Marquee>
        <Marquee reverse pauseOnHover vertical className="[--duration:25s]">{thirdRow.map((r) => <ReviewCard key={r.username} {...r} />)}</Marquee>
        <Marquee pauseOnHover vertical className="[--duration:25s]">{fourthRow.map((r) => <ReviewCard key={r.username} {...r} />)}</Marquee>
      </div>
